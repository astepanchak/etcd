...
# Import keypair. It must match the PEM file we are going to use for SSH.
# We are using a single keypair for both Bastion and etcd nodes.
...
# Internet Gateway resource for DMZ Subnets
...
# Route Tables resource for DMZs via Internet Gateway
...
resource "aws_subnet" "private" {
  count = "${var.node_count}"
  vpc_id = "${aws_vpc.main.id}"
  cidr_block = "${cidrsubnet(var.vpc_cidr, 8, count.index)}" # Private subnets are x.x.[0-2].0/24
  availability_zone = "${element(var.zones, count.index)}"
...
# EIPs for NAT Gateways resource
resource "aws_eip" "nat" {
  count = "${var.node_count}"
  vpc = true
}

# NAT Gateways resource
...
# Route Tables for Private Subnets resource
