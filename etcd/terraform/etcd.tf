...
# Role Policy which allows Instances to update Route53 records
...
# Role Policy which allows Instances to attach EBS volumes.
...
# Securty group which allows all outbound traffic and SSH from the Bastion, and etcd ports internally
...
resource "aws_security_group" "etcdlb" {
  ...
  # etcd client from outside world
  ingress {
    from_port = "${var.etcd_client_port}"
    to_port = "${var.etcd_client_port}"
    protocol = "TCP"
...
# Outputs - DNS name of load balancer, Private IPs of etcd* instances
output "etcd_dns" {
