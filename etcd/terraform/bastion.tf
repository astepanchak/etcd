...
resource "aws_instance" "bastion" {
  ...
  # It requires Identity loaded into SSH Agent with commands in the README
...
# Security Group for incoming SSH and ping from our local management IP
resource "aws_security_group" "bastion" {
  vpc_id = "${aws_vpc.main.id}"
  name = "bastion"
  description = "SSH + ICMP from control CIDR; all outbound"

  # Allow SSH traffic from local management CIDR
  ingress {
    from_port = 22
    to_port = 22
    protocol = "TCP"
  ...
  # Allow ICMP internal and from local mangement IP
  ingress {
    from_port = 8
    to_port = 0
    protocol = "icmp"
...
# Outputs - IPs of Bastion and authorized inbound (management host)
